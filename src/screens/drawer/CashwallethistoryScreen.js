...
const CashwallethistoryScreen = () => {
    ...
    const getCashWalletHistory = async () => {
        const self_id = await AsyncStorage.getItem('user_id');
        if(self_id != ''){
            const data = {
                selfid:self_id,
            };
            axios.post('https://robinjoseph.org/app/api/getCashWalletHistory',data).then(async res => {
                if(res.data.response == 200){
                    setrefreshData(false);
    ...
    const renderItem = ({ item }) => (
        <View style={styles.cardView}>
            <View style={styles.cardTextView}>
                ...
                <Text style={styles.cardText}>{item.child_id}</Text>
            </View>
            <View style={styles.cardTextView}>
    ...
    return(
        <View style={styles.container}>
            <View style={styles.dashContainer}>
                <View>
                    <Text style={styles.pageHeading}>Cash Wallet History</Text>
                </View>
                <View>
                    <FlatList
                    data={listData}
...
export default CashwallethistoryScreen;