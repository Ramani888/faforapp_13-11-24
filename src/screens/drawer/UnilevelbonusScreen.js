...
const UnilevelbonusScreen = () => {
    const [userData, setUserData] = useState('');
    const [listData, setListData] = useState('');
    const [refreshData, setrefreshData] = useState(true);
    useEffect(() => {
        getUserDetail();
        getUnilevelBonusHistory();
    },[refreshData]);
    ...
    const getUnilevelBonusHistory = async () => {
        const self_id = await AsyncStorage.getItem('user_id');
        if(self_id != ''){
            const data = {
                selfid:self_id,
            };
            axios.post('https://robinjoseph.org/app/api/getUnilevelBonusHistory',data).then(async res => {
                if(res.data.response == 200){
                    setrefreshData(false);
    ...
    const renderItem = ({ item }) => (
        <View style={styles.cardView}>
            ...
            <View style={styles.cardTextView}>
                <Text style={styles.cardText}>PV :</Text>
                <Text style={styles.cardText}>{item.pv}</Text>
            </View>
            <View style={styles.cardTextView}>
                <Text style={styles.cardText}>Income :</Text>
                <Text style={styles.cardText}>{item.cash_wallet}</Text>
            </View>
            <View style={styles.cardTextView}>
    ...
    return(
        <View style={styles.container}>
            <View style={styles.dashContainer}>
                <View>
                    <Text style={styles.pageHeading}>Uni-Level Bonus History</Text>
                </View>
...
export default UnilevelbonusScreen;