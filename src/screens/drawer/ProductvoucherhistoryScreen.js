...
const ProductvoucherhistoryScreen = () => {
    const [userData, setUserData] = useState('');
    const [listData, setListData] = useState('');
    const [refreshData, setrefreshData] = useState(true);
    useEffect(() => {
        getUserDetail();
        getProductVoucherHistory();
    },[refreshData]);
    ...
    const getProductVoucherHistory = async () => {
        const self_id = await AsyncStorage.getItem('user_id');
        if(self_id != ''){
            const data = {
                selfid:self_id,
            };
            axios.post('https://robinjoseph.org/app/api/getProductVoucherHistory',data).then(async res => {
                if(res.data.response == 200){
                    setrefreshData(false);
    ...
    const renderItem = ({ item }) => (
        <View style={styles.cardView}>
            ...
            <View style={styles.cardTextView}>
                ...
                <Text style={styles.cardText}>{item.paid_status}</Text>
            </View>
            <View style={styles.cardTextView}>
    ...
    return(
        <View style={styles.container}>
            <View style={styles.dashContainer}>
                <View>
                    <Text style={styles.pageHeading}>Product Wallet Request List</Text>
                </View>
                <View>
                    <FlatList
                    data={listData}
...
export default ProductvoucherhistoryScreen;