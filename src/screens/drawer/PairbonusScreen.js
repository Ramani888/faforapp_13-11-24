...
const PairbonusScreen = () => {
    ...
    const getPairBonusHistory = async () => {
        const self_id = await AsyncStorage.getItem('user_id');
        if(self_id != ''){
            const data = {
                selfid:self_id,
            };
            axios.post('https://robinjoseph.org/app/api/getPairBonus',data).then(async res => {
                if(res.data.response == 200){
                    setrefreshData(false);
    ...
    return(
        <View style={styles.container}>
            <View style={styles.dashContainer}>
                <View>
                    <Text style={styles.pageHeading}>Pair Bonus History</Text>
                </View>
                <View>
                    <FlatList
                    data={listData}
...
export default PairbonusScreen;